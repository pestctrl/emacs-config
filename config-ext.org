...
#+begin_src emacs-lisp
  ...
  (use-package notmuch
    ...
    (defun notmuch-search-tree-current-thread ()
      (interactive)
    ...
    (setq notmuch-search-oldest-first nil
          ...
          '((:name "inbox" :query "tag:inbox" :key "i")
            (:name "inbox today" :query "date:7d.. and tag:inbox" :key "t")
    ...
    (defun run-offlineimap ()
      ...
      (set-process-sentinel
       (start-process-shell-command "offlineimap" nil "offlineimap")
       #'(lambda (process event)
           (when (string-match "^finished" event)
             (message "Offlineimap finished")
             (run-with-timer 300 nil #'run-offlineimap))))))
#+end_src
** erc
#+begin_src emacs-lisp
  (use-package erc)
