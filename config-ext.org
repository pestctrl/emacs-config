  ...
  (use-package notmuch
    ...
    (defun notmuch-search-show-all ()
      ...
      (let* ((query (replace-regexp-in-string "date:[^ ]+" "" notmuch-search-query-string))
             (noand (replace-regexp-in-string "^ *and +" "" noand))
    ...
    (defun notmuch-search-tree-current-thread (arg)
      ...
      (let* ((thread-id (notmuch-search-find-thread-id))
        ...
        (my-notmuch-tree thread-id
                         (unless (zerop (length input)) input)
                         nil nil nil nil nil
                         (unless arg #'notmuch-tree-hide-dead-trees))))

    (defun notmuch-tree-focus (arg)
      (interactive "P")
      (my-notmuch-tree notmuch-tree-basic-query
                       (when arg (notmuch-tree-get-message-id))
                       nil nil nil nil nil
                       (if (not arg) #'notmuch-tree-hide-dead-trees #'notmuch-tree-show-trail-and-alive-children)))
