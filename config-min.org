...
#+begin_src emacs-lisp
  ...
  (unless (string= my/keymap-key (kbd "C-z"))
...
#+begin_src emacs-lisp
  ...
  (use-package banner-comment
    :commands my/banner-comment
    :bind (("C-c h" . #'my/banner-comment))
    :config
    (defun my/banner-comment ()
      (interactive)
      (save-excursion
	(save-restriction
	  (beginning-of-line)
	  (forward-to-indentation 0)
	  (let ((banner-width (- (or banner-comment-width
				     comment-fill-column
				     fill-column)
				 (current-column)))
		...
		(replace-match ;; replace everything after
		 (concat
		  (if (string-empty-p (match-string 98))
		      (make-string remaining-width banner-comment-char)
