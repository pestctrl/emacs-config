...
* firefox window next to org mode
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (defun my/refile-to-point (file point)
    ...
    (let ((ff (get-buffer "fireorg")))
      (if (null ff)
          (progn
            ...
            (run-with-timer 2 nil 'my/youtube-loop))
          ...
          (fireorg/setup org ff
            ...
            (ol/children
             (fireorg/open-link ff
               (when (my/lenient-y-or-n-p "Refile this one?")
      ...
      (if (null ff)
        ...
        (let ((org (find-file-noselect (my/agenda-file "refile.org"))))
          (fireorg/setup org ff
            ...
            (ol/children
             (fireorg/open-link ff
               (when (not (my/lenient-y-or-n-p "Educational?"))
                 (save-excursion
                   (save-window-excursion
                     (my/org-add-tag "watch")
                     (org-todo "TODO")
  ...
  (defun my/insert-category-heading (point heading-name)
          ...
          (progn
            ...
            (let ((tree-string (current-kill 0)))
              (cond ((or (string-match-p "Watch \"" tree-string)
                     ...
                     (my/refile-to-point (buffer-file-name) youtube-marker))
                    ((or (string-match-p "reddit\.com" tree-string)
                         (string-match-p "redd\.it" tree-string))
                     (my/refile-to-point (buffer-file-name) reddit-marker))
                    (t (outline-next-heading))))))))

  (defun my/org-sort-subtree ()
    (interactive)
       ...
       (when (not (member "sorting" (org-get-tags)))
         (let* ((category-name (completing-read "Category? " (mapcar #'car db)))
           ...
           (unless entry ;; New category! 
             (let ((refile-location (my/insert-category-heading beg category-name)))
               (setf (alist-get category db)
                     refile-location)))
           (when-let (location (alist-get category db))
             (my/refile-to-point (buffer-file-name) location)
             (org-get-last-sibling)))))))
  ...
  (defun fireorg/org-sort-subtree ()
    ...
    (let ((ff (get-buffer "fireorg"))
      ...
      (if (null ff)
          (progn
            (fireorg/make-fireorg-window)
            (run-with-timer 2 nil 'fireorg/org-sort-subtree))
        (fireorg/setup org ff
          (let ((db '((1-undecided . nil)))
                (beg (ol/get-bot-marker))
                (path (org-get-outline-path t)))
