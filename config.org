...
#+begin_src emacs-lisp
  ...
  (defvar workspace-list '((scratch0 . 0)
               (scratch1 . 1))
    "My custom workspace list")

  (defun delete-frame-cleanup-workspace-monitor ()
    (let ((wnumber exwm-workspace-current-index))
      (setq workspace-list
        (remove-if (lambda (cell) (= wnumber (cdr cell)))
               workspace-list))
      (assq-delete-all wnumber monitor-list)
  ...
  (defun my/switch-workspace (name)
    (interactive (list (completing-read "Workspace name? "
                    (mapcar #'car workspace-list))))
    (let* ((key (intern name))
       (workspace-number (cdr (assoc key workspace-list))))
      (if workspace-number
  ...
  (defun my/make-new-workspace (name)
    (let ((workspace-number (my/get-empty-workspace)))
      (add-to-list 'workspace-list
           `(,(intern name) . ,workspace-number))
      (add-to-list 'monitor-list
  ...
  (defun my/move-window-to-workspace ()
    ...
    (let* ((key (completing-read "Move to which workspace? "
                 (mapcar #'car workspace-list)))
       (workspace-number (cdr (assoc (intern key) workspace-list))))
  ...
  (defun my/current-workspace ()
    (interactive)
    (let* ((results (remove-if-not (lambda (cell) (= (cdr cell) exwm-workspace-current-index)) workspace-list))
       (res-count (length results)))
      (if (= res-count 0)
      (message "No name for current workspace")
    (message (format "Current Workspace: #%d \"%s\" on %s" exwm-workspace-current-index (car (car results)))))))

  (defun my/workspace-report ()
    (interactive)
    (loop for i in workspace-list
      do (message "Workspace #%d: \"%s\" on \"%s\"" (cdr i) (car i) (cdr (assoc (cdr i) monitor-list)))))
  ...
  (defvar monitor-list '((0 . "eDP1") 
                         (1 . "eDP1")))

  (add-hook 'exwm-init-hook 'my/setup-workspace-monitors)
  ...
  (defun my/update-workspace-monitors ()
    (->> (loop for j in (cl-sort (copy-seq workspace-list) #'< :key 'cdr)
          collect (list (cdr j) (cdr (assoc (cdr j) monitor-list))))
    (flatten-list)
    ...
    (interactive (list (let ((monitors (my/get-screens)))
             (if (= (length monitors) 2)
                 (if (string= (car monitors)
                      (cdr (assoc exwm-workspace-current-index monitor-list)))
                 (cadr monitors)
                   (car monitors))
               (completing-read "Which Monitor? "
                        monitors)))))
    (setf (alist-get exwm-workspace-current-index monitor-list)
      monitor)
    (my/update-workspace-monitors))
  ...
  (add-to-list 'exwm-init-hook
               (lambda ()
                 (advice-add #'exwm-workspace-switch
                             :after
                             #'workspace-switch-record-monitor)))
