...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-capture-templates
        '(("t" "Todo" entry (file "~/MEGA/org/agenda/refile.org")
           "* STUFF %?\n  :PROPERTIES:\n  :CREATED: %U\n  :VIEWING: %a\n  :END:")
          ("r" "Reviews")
          ("ra" "Automatic Review" entry (file+function "~/MEGA/org/entries/reviews.gpg" setup-automatic-review)
           (file "~/MEGA/org/templates/review-interactive-entry.org"))
          ("rw" "Weekly Review" entry (file "~/MEGA/org/entries/reviews.gpg")
           (file "~/MEGA/org/templates/weekly-review-entry-template.org"))
          ("rm" "Make-up Weekly Review" entry (file+function "~/MEGA/org/entries/reviews.gpg" setup-make-up-review)
           (file "~/MEGA/org/templates/review-interactive-entry.org"))
               ...
               (cond ((and (my/is-a-task)
                     ...
                     ((eq (my/get-project-type buffer-file-name
                      ...
                      (when (or (null my/stuck-projects-flag)
                                (< my/stuck-projects-flag subtree-end))
                        (setq my/stuck-projects-flag subtree-end))
                      nil)
                     (t subtree-end))))))
       (byte-compile 'my/show-stuck-projects)
...
**** My own agenda renderer
     #+BEGIN_SRC emacs-lisp
       ...
       (defun org-ql-agenda-function (ignore)
         ...
         (insert
          (save-window-excursion 
            (org-ql-agenda '("~/MEGA/org/agenda/agenda.org")
              (or (and (todo "HABIT")
                       (deadline <= today))
                  (todo "WAIT")
                  (and (todo "TODO")
                       (tags "REWARD")
                       (priority = "A")))
              :super-groups ((:name "Tasks in other courts" :todo "WAIT")
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key projectile-command-map (kbd "C-b") 'counsel-projectile-switch-to-buffer)
